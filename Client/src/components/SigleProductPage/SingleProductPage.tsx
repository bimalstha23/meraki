...
import { product, setCartDialog, setCurrentProduct, setLoginDialog } from '../../Redux/Reducer'
...
export const SingleProductPage = () => {
    ...
    // const { name, price, rating, id, Image } = data?.product;
    const [open, setOpen] = useState(false);

    const datarating = data?.product.rating;
            ...
            <div className='flex flex-col justify-center' >
                ...
                <div className='flex flex-row gap-5 mx-52 my-10 p-5 rounded-lg shadow-lg'>
                    <div className='w-1/2 rounded-lg '>
                        <ImageSwiper Image={data?.product.Image} />
                    </div>
                    {data ? (
                        ...
                        <div className='flex flex-col mt-12 gap-4 '>
                            ...
                            <div className='flex flex-row items-center gap-2'>
                                ...
                                <p className='text-gray-500 text-sm'>{data.product?.numReviews} (Reviews)</p>
                            </div>
                            <p className='text-gray-500 text-sm'>In Stock</p>
                            <div className='flex flex-row items-center gap-6'>
                                <h1 className='text-lg font-normal'>{convertCurrency(data.product?.price)}</h1>
                                <h1 className='text-lg font-normal line-through'>{data?.actualPrice ? convertCurrency(data?.actualPrice) : ''}</h1>
                            </div>
                            <p className='text-gray-500 text-sm'>{data.product?.description}</p>
                            <button
                            ...
                            <h1 className='text-lg font-normal '> <span className='font-bold'>Category:</span>  {data?.product?.Category}</h1>
                            <div className='flex flex-row gap-3 items-center'>
                                <h1 className='text-lg font-bold'>Tags: </h1>
                                <div className='flex flex-row gap-2'>
                                    {data?.product?.tags?.map((tag: any, key: number) => (
